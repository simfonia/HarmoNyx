...
void setup() {
  ...
  if (!instrumentADSR.containsKey("Synth")) instrumentADSR.put("Synth", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    ...
    instrumentADSR.put("Synth", new float[]{(float)0.01, (float)0.1, (float)0.3, (float)0.4});
          ...
          try {
            ...
            new Thread(new Runnable() {
              public void run() {
                ...
                if (rawPattern.contains(",")) { String[] parts = rawPattern.split(","); for(String p : parts) parsed.add(p.trim()); }
                else { String raw = rawPattern.replace("|", " "); StringBuilder buf = new StringBuilder(); for (int i=0; i<raw.length(); i++) { char c = raw.charAt(i); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString()); }
                String[] steps = parsed.toArray(new String[0]); float stepMs = (60000 / bpm) / 4;
                for (int i=0; i<Math.min(steps.length, 16); i++) {
                  ...
                  int sustainSteps = 1; for (int j=i+1; j<Math.min(steps.length, 16); j++) { if (steps[j].trim().equals("-")) sustainSteps++; else break; }
                  float noteDur = stepMs * sustainSteps;
                  if (!token.equals("-")) {
                    if (instrumentMap.getOrDefault("Kick", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("Kick", 1.0f); samplerGainMap.get("Kick").setValue(map(floatVal(110) * volScale, 0, 127, -40, 0)); samplerMap.get("Kick").trigger(); } }
                    else { if (false) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("Kick", token, noteDur * 0.9f, floatVal(110)); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Kick", midi, floatVal(110), noteDur * 0.9f); } }
                    else { if (token.equalsIgnoreCase("x")) playNoteForDuration("Kick", 60, floatVal(110), noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Kick", midi, floatVal(110), noteDur * 0.9f); } } }
                  }
            ...
            new Thread(new Runnable() {
              public void run() {
                ...
                for (int i=0; i<Math.min(steps.length, 16); i++) {
                  ...
                  float noteDur = stepMs * sustainSteps;
                  if (!token.equals("-")) {
                    if (instrumentMap.getOrDefault("Snare", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("Snare", 1.0f); samplerGainMap.get("Snare").setValue(map(floatVal(100) * volScale, 0, 127, -40, 0)); samplerMap.get("Snare").trigger(); } }
                    else { if (false) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("Snare", token, noteDur * 0.9f, floatVal(100)); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Snare", midi, floatVal(100), noteDur * 0.9f); } }
                    else { if (token.equalsIgnoreCase("x")) playNoteForDuration("Snare", 60, floatVal(100), noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Snare", midi, floatVal(100), noteDur * 0.9f); } } }
                  }
                  try { Thread.sleep((long)stepMs); } catch (Exception e) {}
                }
              }
            ...
            new Thread(new Runnable() {
              public void run() {
                ...
                for (int i=0; i<Math.min(steps.length, 16); i++) {
                  ...
                  float noteDur = stepMs * sustainSteps;
                  if (!token.equals("-")) {
                    if (instrumentMap.getOrDefault("HiHat", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("HiHat", 1.0f); samplerGainMap.get("HiHat").setValue(map(floatVal(50) * volScale, 0, 127, -40, 0)); samplerMap.get("HiHat").trigger(); } }
                    else { if (false) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("HiHat", token, noteDur * 0.9f, floatVal(50)); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("HiHat", midi, floatVal(50), noteDur * 0.9f); } }
                    else { if (token.equalsIgnoreCase("x")) playNoteForDuration("HiHat", 60, floatVal(50), noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("HiHat", midi, floatVal(50), noteDur * 0.9f); } } }
                  }
                  try { Thread.sleep((long)stepMs); } catch (Exception e) {}
                }
              }
